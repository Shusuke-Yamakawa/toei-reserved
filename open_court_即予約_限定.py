...
class GetOpenCourt:
    ...
    # TARGET_COURT = ['井の頭恩賜公園', '野川公園', '小金井公園', '府中の森公園', '武蔵野中央公園', '東大和南公園']
    # TARGET_COURT = ['井の頭恩賜公園', '野川公園', '小金井公園', '府中の森公園', '武蔵野中央公園']
    # TARGET_COURT = ['井の頭恩賜公園', '野川公園', '小金井公園', '武蔵野中央公園']
    # TARGET_COURT = ['井の頭恩賜公園', '小金井公園', '武蔵野中央公園']
    TARGET_COURT = ['井の頭恩賜公園']
    # USER_ID = "86329044" "86329053" "86560751" "86757664" "86757673"87242690
    # PASSWD = "19870513" "19470602" "19550223" "19820914" "19890825"19741013

    USER_ID = "86757664"
    PASSWD = "19820914"

    RETRY_USER_ID = "87242690"
    RETRY_PASSWD = "19741013"

    RETRY2_USER_ID = "86757673"
    RETRY2_PASSWD = "19890825"

    GET_LIMIT_DAY = 4
    NOTIFY_OPEN_COURT = 4

    def __init__(self, from_time, to_time, court, target_day, headless):
        if headless == '0':
            options = Options()
    ...
    def run(self):
        try:
            ...
            msg += "https://yoyaku.sports.metro.tokyo.lg.jp/"
    ...
    def search_by_target_day(self, year, month, day):
        ...
        for day in self._handle_dates(year, month, day):
        	day_type_list = {"Holiday":DayJudgeHoliday, "Wednesday":DayJudgeWednesday, \
        	    "MiddleWeek":DayJudgeMiddleWeek, "Saturday":DayJudgeSaturday, "Sunday":DayJudgeSunday}
    ...
    def now_reserve(self, msg, year, month, retry):
        ...
        if retry:
            if msg.count('重複してます') == 1:
                self.driver.find_element_by_id('userid').send_keys(self.RETRY_USER_ID)
                self.driver.find_element_by_id('passwd').send_keys(self.RETRY_PASSWD)
            elif msg.count('重複してます') == 2:
                self.driver.find_element_by_id('userid').send_keys(self.RETRY2_USER_ID)
                self.driver.find_element_by_id('passwd').send_keys(self.RETRY2_PASSWD)
            else:
                msg += '全て重複しています。'
                return
        else:
        ...
        if len(web_elements) > 0:
            for e in web_elements:
                ...
                if(self.target_court(e.text)):
                    ...
                    icon_elements = self.driver.find_elements_by_id('emptyStateIcon')
...
if __name__ == "__main__":
    ...
    def main(args):
        if len(args) == 6:
            GetOpenCourt(args[1], args[2], args[3], args[4], args[5]).run()
        else:
            GetOpenCourt(15, 17, "人工芝", "Wednesday", "1").run()
    main(sys.argv)